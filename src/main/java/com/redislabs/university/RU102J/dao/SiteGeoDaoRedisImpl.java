...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            Set<Response<Map<String, String>>> siteHashes = new HashSet<>(keys.size());
            Pipeline pipeline = jedis.pipelined();
            for (String key : keys) {
                siteHashes.add(pipeline.hgetAll(key));
            }
            pipeline.sync();;
            for (Response<Map<String, String>> siteHash : siteHashes) {
                if (!siteHash.get().isEmpty()) {
                    sites.add(new Site(siteHash.get()));
                }
            }
            return sites;
        }
    ...
    private Set<Site> findSitesByGeoWithCapacity(GeoQuery query) {
        ...
        GeoUnit radiusUnit = query.getRadiusUnit();
         ...
         try (Jedis jedis = jedisPool.getResource()) {
             ...
             List<GeoRadiusResponse> radiusResponses = jedis.georadius(RedisSchema.getSiteGeoKey(), coord.getLng(), coord.getLat(), radius, GeoUnit.KM);
